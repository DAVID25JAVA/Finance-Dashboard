...
export default function Insights() {
  ...
  const getInsights = () => {
    ...
    // Average transaction
    const avgTransaction = (transactions.reduce((sum, t) => sum + Math.abs(t.amount), 0) / transactions.length).toFixed(2);
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-3 p-3 bg-gray-700/30 rounded-lg">
          ...
          <div>
            ...
            <p className="text-lg font-semibold text-white">{insights.topCategory?.[0] || 'N/A'}</p>
            <p className="text-sm text-gray-400">${insights.topCategory?.[1]?.toFixed(2) || '0'}</p>
          </div>
        </div>
          ...
          <div>
            ...
            <p className="text-lg font-semibold text-white">
              {insights.monthlyChange > 0 ? (
                <span className="text-red-400">↑ {insights.monthlyChange}%</span>
              ) : (
                <span className="text-green-400">↓ {Math.abs(insights.monthlyChange)}%</span>
              )}
            </p>
            <p className="text-sm text-gray-400">vs last month (${insights.currentMonthExpenses?.toFixed(2) || '0'})</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 p-3 bg-gray-700/30 rounded-lg">
          ...
          <div>
            ...
            <p className="text-lg font-semibold text-white">${insights.avgTransaction}</p>
          </div>
        </div>
      </div>
    </div>
